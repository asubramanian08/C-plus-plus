...
//pro. ex. q6
template <class T>
T maxn(const T arr[], unsigned int n_el)
{
	T greatest;
	if (n_el != 0)
	{
...
template <>
char *maxn<char *>(char *const arr[], unsigned int n_el)
{
	if (n_el == 0)
		return NULL;
	char *longest = arr[0];
...
int main(void)
{
	...
	while (true)
	{
		cout << "Enter a string to capitoline "
			 << "(q to quit): ";
		cin >> str;
		if (str.compare("q") != 0)
		{
	...
	cout << "String capitalization game complete!" << endl
		 << endl;
	...
	delete[] arr_doub;
	cout << "Greatest in array of size " << ARR_LEN << " (max5) test complete!" << endl
		 << endl;
...
#define MAX_STR 100
	...
	for (unsigned int i = 0; i < STR_ARR_LEN; i++)
		delete[] str_arr[i];
	delete[] str_arr;
	cout << "Greatest in array of size n (maxn) test complete!" << endl;
